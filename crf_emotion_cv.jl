...
for i = 1:nl
    ...
    model = train_tree_crf(TS_tr, XS_node_tr, XS_edge_tr, YS_tr, nc, lm,   
            alg = alg, step = step, verbose = true, iter_check = 500, max_iter=1_000_000, max_pass = 50)

    YS_pred = predict_tree_crf(model, TS_val, XS_node_val, XS_edge_val, loss, LS_weights_val)
    avg_loss, v_loss = test_tree_crf(YS_pred, YS_val, loss, LS_weights_val)
    # avg_loss = nodewise_test_tree_crf(YS_pred, YS_val, loss, LS_weights_val)
...
v_model = Vector{CRFTreeModel}(20)
...
for i = 1:20
    ...
    model = train_tree_crf(TS_train, XS_node_train, XS_edge_train, YS_train, nc, best_lambda,   
    alg = alg, step = step, verbose = true, iter_check = 500, max_iter=1_000_000, max_pass = 100)

    YS_pred = predict_tree_crf(model, TS_test, XS_node_test, XS_edge_test, loss, LS_weights)

    avg_loss, v_loss = test_tree_crf(YS_pred, YS_test, loss, LS_weights_test)
    nodewise_avg_loss = nodewise_test_tree_crf(YS_pred, YS_test, loss, LS_weights_test)
...
fid = h5open("results/CRF-emotion-" * string(loss) * "-w" * string(weighted) * ".h5", "w")
